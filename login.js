(function() {
    class SupabaseAuthManager {
        constructor() {
            ...
            // شبكة الأمان: إظهار الصفحة بالقوة بعد 3.5 ثانية
            ...
            try {
                ...
                if (av && photoUrl) {
                    tasks.push(new Promise(resolve => {
                        ...
                        av.onload = () => { clearTimeout(timeout); resolve(); };
                        av.onerror = resolve;
                        av.src = photoUrl;
                    }));
                }
        ...
        async handleSessionSync(user) {
            ...
            const fingerprint = 'fp_' + btoa(navigator.userAgent).substring(0, 10);
            const ip = await this.fetchIP();
            const { data } = await this.supabase.from('sessions').upsert({ 
                user_id: user.id, fingerprint, ip, os: this.getOS(), last_active: new Date().toISOString() 
            }).select();
        ...
        setupGoogleOneTap() {
            try {
                ...
                // تنظيف حالة جوجل القديمة لإجبار المربع على الظهور دائماً
                ...
                google.accounts.id.initialize({
                    ...
                    auto_select: false,
                    use_fedcm_for_prompt: true, // يمنع أخطاء الكونسول ويرفع درجة PageSpeed
                    itp_support: true
                });
